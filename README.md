...
7D2D 2.x zombies have melee ranges between 1.3m and 2.2m. Combined with server/client desync, that produces the "how did they hit me from over there" feeling. This mod tightens horizontal reach on every zombie hand item — including the new Rancher and Chuck zombies, HazMats, Workers, and Boars — while nerfing crawlers a little further since they're low to the ground anyway.
...
`Extends` chains mean Feral/Charged/Infernal variants that don't override `Range` inherit from their parent, so patching the parent covers them automatically. Rancher, Chuck, Cop, and Mutated variants all cascade this way. HazMat/Worker/PartyGirl get explicit patches (via `append`) because they don't have their own `Range` property in vanilla — they silently inherited Master's 1.65.

## What it does NOT touch

- `meleeHandZombieStrong` / `Demolition` (3.1m shoulder charge — intentional)
- `meleeHandBossGrace` (2.4m, named boss)
- Passive animals

## Credits

Inspired by [KhaineGB's Zombie Reach Adjustment](https://www.nexusmods.com/7daystodie) — the idea of tightening zombie reach to make melee feel fair is his. Values and item list in this mod are our own (researched against current 2.x `items.xml`), and coverage has been extended to include the new Rancher and Chuck zombies, HazMats, Workers, PartyGirls, Boars, and crawlers — which the original doesn't cover.
